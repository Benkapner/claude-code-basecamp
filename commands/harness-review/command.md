...
Read the actual content of every component discovered by the lint inventory. Cover all tool layouts present in the project, including for example: `.cursor/rules/*.mdc`, `.cursorrules`, `.cursor/commands/*.md`, `.cursor/hooks.json`, `CLAUDE.md`, `AGENTS.md`, `commands/`, `skills/**/SKILL.md` (including reference files in subdirectories), `.claude/`, and MCP configs.
...
At the end of the report, include: `Evaluated with: harness-eval v{version} (workspace-command)` where {version} comes from `uvx --from harness-eval harness-eval --version` or `pip show harness-eval`.
